    ...
    this.audio = {
      onplay() {
        ...
        window.timeIntervalId = window.setInterval(() => {
          const text = `${window.clientLibrary.currentTimeDisplay} / ${window.clientLibrary.durationDisplay}`;

          window.setProgressText(
            text.indexOf('NaN') === -1 ? text : '',
          );
        }, 1000);
      ...
      onended() {
        ...
        const nextTrack = document.querySelector('.playing')
          .nextElementSibling;
    ...
    this.progressContainer = {
      onmousemove(event) {
        if (!window.audio.paused) {
          ...
          const {
            width: totalWidth,
          } = window.progressContainer.getBoundingClientRect();
      ...
      onmousedown(event) {
        ...
        window.audio.currentTime =
          ...
          parseFloat((x / totalWidth).toFixed(4));
      },
    };
  }
}
