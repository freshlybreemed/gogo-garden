export default class ClientLibrary {
  ...
  DOMSetup() {
    ...
    window.progressContainer.onmousemove =
      window.eventLibrary.progressContainer.onmousemove;
    ...
    window.setProgressText = (text) =>
      (window.progressTime.innerHTML = text);

    return this;
  }

  gatherElements() {
    window.progressContainer = document.querySelector(
      '.progress__container',
    );
    window.progressIndicator = document.querySelector(
      '.progress__cursor',
    );
  ...
  registerTrackTime() {
    window.audio.ontimeupdate =
      window.eventLibrary.audio.ontimeupdate;
    window.progressContainer.onmousedown =
      window.eventLibrary.progressContainer.onmousedown;
  }
  ...
  setNowPlaying(cRow) {
    let clickedRow = cRow.target.parentElement;
    console.log(cRow, clickedRow);
  ...
  togglePausePlay() {
    ...
    if (pauseOrPlay.innerHTML !== 'equalizer') {
      pauseOrPlay.innerHTML === 'play_arrow'
        ? ((pauseOrPlay.innerHTML = 'pause'), window.audio.play())
        : ((pauseOrPlay.innerHTML = 'play_arrow'),
          window.audio.pause());
    }
  }
